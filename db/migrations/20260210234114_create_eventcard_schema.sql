...
-- Trigger for Search
CREATE OR REPLACE FUNCTION event_cards_search_vector_trigger() RETURNS trigger AS $$
BEGIN
  NEW.search_vector :=
...
CREATE OR REPLACE FUNCTION get_event_cards_by_event_id(p_event_id UUID)
RETURNS TABLE
(
  id UUID,
  event_id UUID,
...
AS $$
BEGIN
  INSERT INTO event_cards (event_id, guest_name, access_token)
    VALUES (p_event_id, p_guest_name, p_access_token)
    RETURNING id;
END;
$$;
...
CREATE OR REPLACE FUNCTION delete_event_card(p_id UUID)
RETURNS VOID
LANGUAGE plpgsql
AS $$
BEGIN
