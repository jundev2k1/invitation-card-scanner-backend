...
CREATE OR REPLACE FUNCTION get_event_members_by_event_id(p_event_id UUID)
RETURNS TABLE (
  id UUID,
  event_id UUID,
  user_id UUID,
...
BEGIN
  RETURN QUERY
  SELECT  id, event_id, user_id, assigned_role, assigned_at
    FROM  event_members
...
AS $$
BEGIN
  INSERT INTO event_members(event_id, user_id, assigned_role)
  VALUES (p_event_id, p_user_id, p_assigned_role);
END;
$$;
...
CREATE OR REPLACE FUNCTION delete_event_member(p_id UUID)
RETURNS VOID
LANGUAGE plpgsql
AS $$
BEGIN
